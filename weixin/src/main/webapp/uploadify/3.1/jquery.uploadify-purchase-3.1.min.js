...
(function (c) {
    var b = {init: function (d, e) {
        return this.each(function () {
            var n = c(this);
            var m = n.clone();
            var j = c.extend({id: n.attr("id"), swf: "/uploadify/3.1/uploadify.swf", uploader: "", auto: false, buttonClass: "", buttonCursor: "hand", buttonImage: "/uploadify/3.1/uploadfile_btn.png", buttonText: "", checkExisting: false, debug: false, fileObjName: "Filedata", fileSizeLimit: 5120, fileTypeDesc: "All Files", fileTypeExts: "*.*", height: 25, method: "post", multi: true, formData: {}, preventCaching: true, progressData: "percentage", queueID: false, queueSizeLimit: 5, removeCompleted: false, removeTimeout: 0, requeueErrors: false, successTimeout: 30, uploadLimit: 5, width: 66, overrideEvents: []}, d);
            ...
            if (h) {
                ...
                i.queueData = {files: {}, filesSelected: 0, filesQueued: 0, filesReplaced: 0, filesCancelled: 0, filesErrored: 0, uploadsSuccessful: 0, uploadsErrored: 0, averageSpeed: 0, queueLength: 0, queueSize: 0, uploadSize: 0, queueBytesUploaded: 0, uploadQueue: [], errorMsg: "有些文件没有被添加到队列中："};
                i.original = m;
                i.wrapper = l;
                i.button = f;
                i.queue = k;
        ...
        this.each(function () {
            ...
            if (e[0]) {
                if (e[0] == "*") {
                    ...
                    var len= e.length;
                    var cThis = c(this);
                    if (len == 1 && typeof (c("#" + e[len - 1]).find('input').val()) != "undefined") {
                        i.cancelUpload(e[len - 1]);
                        var pThisType = c("#" + e[len - 1]).parents(".received-box").attr("data-index");
                        if (pThisType == "1") {
                            $.ajax({url: "/center/album/picture/delete.htm", dataType: "json", type: "post", data: {id: c("#" + e[len - 1]).find('input[name="imgId"]').val()}, success: function (data, textStatus) {
                                if (data.resultInfo == true) {
                                    c("#" + e[len - 1]).find(".data").removeClass("data").html(" - 删除中");
                                    c("#" + e[len - 1]).find(".uploadify-progress-bar").remove();
                                    c("#" + e[len - 1]).delay(1000 + 100 * len - 1).fadeOut(500, function () {
//                                    //cThis.remove();
//
                                        var fileId="",queueId="",pId="";
                                        fileId=c("#" + e[len - 1]).parent().prev().attr("id");
                                        queueId=c("#" + e[len - 1]).parent().attr("id");
                                        queue=c("#" + e[len - 1]).parent();
                                        _a=c("#" + e[len - 1]).parent().siblings("a");
                                        pId=c("#" + e[len - 1]).parents(".received-box").attr("id");
                                        var ps = c("#" + e[len - 1]).parents(".received-box");
                                        /*console.log("fileId: " + fileId + " queueId: " + queueId + " pId: " + pId);*/
                                        if(ps.find(".init-edit:visible").size()==0){
                                            ps.find("a.fileupload1").hide();
                                        }
                                        uploadInitImg(fileId,queueId,pId);
                                    });

                                } else if (data.resultInfo == false) {
                                    c("#" + e[len - 1]).find(".data").removeClass("data").html(" - 删除失败");
                                } else {
                        ...
                        $.ajax({url: "/center/attachment/delete.htm", dataType: "json", type: "post", data: {id: c("#" + e[len - 1]).find('input[name="attachId"]').val()}, success: function (data, textStatus) {
                            if (data.resultInfo == true) {
                                c("#" + e[len - 1]).find(".data").removeClass("data").html(" - 删除中");
                                c("#" + e[len - 1]).find(".uploadify-progress-bar").remove();
                                c("#" + e[len - 1]).delay(1000 + 100 * len - 1).fadeOut(500, function () {
...
//
                                    ...
                                    /*console.log("fileId: " + fileId + " queueId: " + queueId + " pId: " + pId);*/
                                    if(ps.find(".init-edit:visible").size()==0){
                                        ps.find("a.fileupload").hide();
                                    }
                                    uploadInitAttach(fileId,queueId,pId);
                                });
                        ...
                        for (var m = 0; m < e.length; m++) {
                            ...
                            i.cancelUpload(e[m]);
                            var pThis=c("#" + e[m]).parents(".received-box");
                            c("#" + e[m]).find(".data").removeClass("data").html(" - 取消中");
                            c("#" + e[m]).find(".uploadify-progress-bar").remove();
                            var cType = c("#" + e[m]).parents(".received-box").attr("data-index");
                            c("#" + e[m]).delay(1000 + 100 * m).fadeOut(500, function () {
                                if(pThis.find(".init-edit:visible").size()==0){
                                    if (cType == "1") {
                                        pThis.find("a.fileupload1").hide();
                                    }
                                    else {
                                        pThis.find("a.fileupload").hide();
                                    }
                                }
                                //c(this).remove();
                            });
    ...
    var a = {onDialogOpen: function () {
        ...
        if (c.inArray("onSelect", h.overrideEvents) < 0) {
            c("#" + h.queueID).append('<div id="' + g.id + '" class="uploadify-queue-item init-edit">					<div class="cancel">						<a href="javascript:$(\'#' + h.id + "').uploadify('cancel', '" + g.id + '\')">X</a>					</div>					<span class="fileName">' + j + " (" + i + ')</span><span class="data"></span>					<div class="uploadify-progress">						<div class="uploadify-progress-bar"><!--Progress Bar--></div>					</div>				</div>');
        }
            ...
            switch (g) {
                case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
                    if (e.queueSizeLimit > f) {
                        this.queueData.errorMsg += '\n选定的文件数超过上传限制,最多能选择 (' + f + ') 个文件';
                    } else {
                        this.queueData.errorMsg += '\n选定的文件数超过上传限制,最多能选择(' + e.queueSizeLimit + ') 个文件';
                    }
                    break;
                case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                    this.queueData.errorMsg += '\n选定的文件 "' + d.name + '"  超过大小限制 (' + e.fileSizeLimit + ")";
                    break;
                case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
                    this.queueData.errorMsg += '\n选定的文件 "' + d.name + '" 大小为零';
                    break;
                case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                    this.queueData.errorMsg += '\n选定的文件 "' + d.name + '" 是不接受的文件类型 (' + e.fileTypeDesc + ")";
                    break;
            }
        }
        ...
        var getData = eval('(' + h + ')');
        if (getData.id) {
            c("#" + f.id).append('<input type="hidden" name="attachId" value="' + getData.id + '"/>');
        }
        if (getData.resultInfo.isSuccess) {
            c("#" + f.id).append('<input type="hidden" name="imgId" value="' + getData.resultInfo.id + '"/>');
        }
