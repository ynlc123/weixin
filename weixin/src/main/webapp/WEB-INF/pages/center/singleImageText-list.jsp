...
<body>
	...
	<div class="container content">
	    <h6 style="font-weight: bold;display:inline;">单图文回复列表</h6>
	    <a href="/center/singleImageText/new.htm" class="btn btn-primary btn-sm pull-right">新增单图文回复</a>
	    <table class="table table-hover table-striped">
	        ...
	        <tbody>
	        <c:forEach items="${pageView.records }" var="imageText" varStatus="status">
	        <tr>
	            <td>${status.index+1 }</td>
	            <td><a href="/view/singleImageText/detail.htm?uuid=${imageText.uuid }" target="_blank">${imageText.keywords }</a></td>
	            <td><fmt:formatDate value="${imageText.createTime}" pattern="yyyy-MM-dd" /></td>
	            <td>
	            	<c:if test="${imageText.status==1 }"><i style="font-style:normal;">正常</i></c:if>
		            <c:if test="${imageText.status==0 }"><i style="font-style:normal;color:#ff0000;">已禁用</i></c:if>
	            </td>
	            <td data-id="${imageText.uuid }">
	            	<c:if test="${imageText.status==1 }"><a href="javascript:;" class="btn btn-warning btn-xs forbid">禁用</a></c:if>
		            <c:if test="${imageText.status==0 }"><a href="javascript:;" class="btn btn-success btn-xs enable">启用</a></c:if>
		            <a href="/center/singleImageText/edit.htm?uuid=${imageText.uuid }" target="_blank" class="btn btn-info btn-xs">编辑</a>
	        ...
	        <tr>
	        	<td colspan="5">
	        		<div class="alert alert-warning text-center" style="height: 100px;line-height: 100px;" role="alert">
	        			您还没有发不过文本回复！<a href="/center/singleImageText/new.htm" class="btn btn-primary btn-sm">新增单图文回复</a>
	        		</div>
	        	</td>
	        </tr>
	        </c:if>
	...
	<script src="/js/singleImageText_list.js"></script>
</body>
</html>