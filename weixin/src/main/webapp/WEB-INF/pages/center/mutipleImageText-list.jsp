...
<body>
	...
	<div class="container content">
	    <h6 style="font-weight: bold; display:inline;">多图文回复列表</h6>
	    <a href="/center/mutipleImageText/new.htm" class="btn btn-primary btn-sm pull-right">发布多图文</a>
	    <table class="table table-hover table-striped">
	        ...
	        <tbody>
	        <c:forEach items="${pageView.records }" var="mutipleImageText" varStatus="status">
	        <tr>
	            <td>${status.index+1 }</td>
	            <td><a href="#" title="${mutipleImageText.keywords }">${mutipleImageText.keywords }</a></td>
	            <td><fmt:formatDate value="${mutipleImageText.createTime}" pattern="yyyy-MM-dd" /></td>
	            <td>
	            	<c:if test="${mutipleImageText.status==1 }">正常</c:if>
	            	<c:if test="${mutipleImageText.status==0 }"><i style="font-style:normal;color:#ff0000;">已禁用</i></c:if>
	            </td>
	            <td data-id="${mutipleImageText.uuid }">
	            	<c:if test="${mutipleImageText.status==1 }"><a href="javascript:;" class="btn btn-warning btn-xs forbid">禁用</a></c:if>
		            <c:if test="${mutipleImageText.status==0 }"><a href="javascript:;" class="btn btn-success btn-xs enable">启用</a></c:if>
		            <a href="/center/mutipleImageText/edit.htm?uuid=${mutipleImageText.uuid }" target="_blank" class="btn btn-info btn-xs">编辑</a>
	        ...
	        <tr>
	        	<td colspan="7">
	        		<div class="alert alert-warning text-center" style="height: 100px;line-height: 100px;" role="alert">
	        			您还没有多图文回复哦！<a href="/center/mutipleImageText/new.htm" class="btn btn-primary btn-sm">发布多图文</a>
	        		</div>
	        	</td>
	        </tr>
	        </c:if>
	...
	<script src="/js/mutipleImageText_list.js"></script>
</body>
</html>